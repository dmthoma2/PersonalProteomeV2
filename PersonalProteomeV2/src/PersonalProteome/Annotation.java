...
import Gene.CDS;
import Gene.DataPoint;
import Gene.Mutation;
import Gene.RegionOfInterest;
import Gene.Seleno;
import Gene.Transcript;
import Gene.VariantPoint;
...
public class Annotation{
	 ...
	 */
	public void populateGTFList(String GTFFileLocation){
	...
	private void createOutputFiles(String outputFileName, String dataPointsFileName, String statsOutputFile){
		try {
				...
				ArrayList<VariantPoint> variantData;
				...
				//BedFileOutput related variables
				StringBuffer sbBed;
				...
				for(int i = 0; i < transcriptList.size(); i++){
					...
					ArrayList<Integer> variantLocs = new ArrayList<Integer>();
					ArrayList<String> variantSNP = new ArrayList<String>();
					ArrayList<String> variantREF = new ArrayList<String>();
					...
					for(int n = 0; n < limit; n++){
						...
						if(protein.charAt(n) != refProtein.charAt(n)){
							...
							variantSNP.add(charToLower);
							variantREF.add(refProtein.substring(n, n+1));
					...
					//Determine how many nonsense codons are in this protein
					int nonsenseCount = 0;
					
					for(int y = 0; y < protein.length(); y++){
						if(protein.substring(y, y+1).equals(Definitions.STOP_CODON_STRING)){
							nonsenseCount++;
						}
					}
					//Create a line for the new variant data point
					variantData.add(new VariantPoint(geneName + "|" + transcriptName, numOfVariants, protein.length(), nonsenseCount));
					
					
					/*Write a bed file line for this protein if it has any variants*/
					...
					for(int q = 0; q < t.getvariantCount(); q++){
						...
						//The genomic strand that this peptide belongs on
						 ...
						 int peptideLocationInProtein = variantLocs.get(q);
						 int peptideStopLocationInProtein = peptideLocationInProtein + 1;
...
//								 U.p("Split Location " + k + " is: " + beingWorkedOn.getExonSplitLocations().get(k));
							...
							//Determine how many bp the peptide goes into the given CDS
							 ...
							 int peptideStopLoc = peptideStartLoc + (1 * 3);
							 ...
							 if(currentStrand == Definitions.genomicStrandNEGATIVE){
								 ...
								 peptideStartLoc = peptideStopLoc - (1 * 3);
						...
						//Get color of this bed line

						 
						 /*
						  * Color codes
						  * 
						  *Stop -> Non Stop  YELLOW
						  * Non Stop -> Stop RED
						  * Meth -> Non Meth BLUE
						  * Non Meth -> Meth Green
						  * All else BLACK
						 ...
						 String color = "0,0,0";
								...
								//stop -> non stop  YELLOW
						 		if(variantREF.get(q).equals(Definitions.STOP_CODON_STRING)){
						 			
						 			if(!variantSNP.get(q).equals(Definitions.STOP_CODON_STRING)){
						 				color = "255,255,0";
						 			}
						 			
						 		//non stop -> stop  RED
						 		}else if(!variantREF.get(q).equals(Definitions.STOP_CODON_STRING)){
						 		...
						 		//M -> not meth  BLUE
						 		}else if(variantREF.get(q).equals("M")){
		
					 				if(!variantSNP.get(q).equals("M")){
					 					color = "0,0,255";
					 				}
						...
						//Create a bed line for each variant
						sbBed.append( Definitions.convertChrmNumToString(GTFlist.get(t.getiD()).getChromosomeName()) + "\t" + startLoc + "\t" + stopLoc + "\t" +
								variantREF.get(q) + "-->" + variantSNP.get(q) + "\t" + 1000 + "\t" + Definitions.convertStrandToString(GTFlist.get(t.getiD()).getGenomicStrand()) +
								"\t" + startLoc + "\t" + stopLoc + "\t" + color + "\n");
...
//					sb.append("\n");
			...
			//Now write the bed file to disk
			out = new BufferedWriter(new FileWriter(outputDir + "SNPS_BED_FILE.bed"));
			
			out.write("track itemRgb='On'" + "\n");
			out.write(sbBed.toString());
			...
			Collections.sort(variantData);
			//Create a output file
			BufferedWriter variantDataOut= new BufferedWriter(new FileWriter(outputDir + Properties.variatnDataFileName));
			
			variantDataOut.write("GeneName|TranscriptName" + "\t" + "VariantCount" + "\t" + "PercentOfAcidsVariant" + "\t" + "VariantCountSquared/Length" + "\t" + "NonsenseCodons" + "\n");
			for(VariantPoint vp: variantData){
				variantDataOut.write(vp.toString());
				variantDataOut.newLine();
			}

			//Calculate statistics
			//Close the output file
			variantDataOut.flush();
			variantDataOut.close();
...
//				out.write(">sp|" + g.getTranscript_Name() + "\n");
				out.write(">");//No Header, used in MSGF Testing
