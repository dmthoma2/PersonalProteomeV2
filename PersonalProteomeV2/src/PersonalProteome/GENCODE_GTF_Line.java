...
public class GENCODE_GTF_Line implements Comparable<GENCODE_GTF_Line>{
	...
	@Override
	/**
	 * Sorts based on chromosome, start, and finally stop
	 */
	public int compareTo(GENCODE_GTF_Line gtfLine) {
		if(this.chromosomeName < gtfLine.getChromosomeName()){
			return -1;
		}
		if(this.chromosomeName > gtfLine.getChromosomeName()){
			return 1;
		...
		if(this.startLocation < gtfLine.getStartLocation()){
			return -1;
		}
		if(this.startLocation > gtfLine.getStartLocation()){
			return 1;
		...
		if(this.stopLocation < gtfLine.getStopLocation()){
			return -1;
		}
		if(this.stopLocation > gtfLine.getStopLocation()){
			return 1;
