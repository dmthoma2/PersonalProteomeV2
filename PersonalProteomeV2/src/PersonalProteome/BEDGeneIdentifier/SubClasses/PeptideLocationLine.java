...
/**
 * PeptideLocationLine represents a single line from a peptide file.  This is used when labeling peptides based on their location.  It preserves information from the input file, and its toString()
...
//		this.score5 = score5;
	...
	/**
	 * Stores the index in the GTFlist 
	 * @param id
	 */
	...
	public int compareTo(PeptideLocationLine p) {
		...
		if(this.stopLocation < p.getStopLocation()){
			return -1;
		}
		if(this.stopLocation < p.getStopLocation()){
			return 1;
		...
		//Sort based on score if all else is equal
		if(this.getStartLocType() + this.getStopLocType() > p.getStartLocType() + p.getStopLocType()){
			return -1;
		}
		if(this.getStartLocType() + this.getStopLocType() < p.getStartLocType() + p.getStopLocType()){
		return 1;
