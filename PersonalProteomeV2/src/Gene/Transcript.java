	...
	public void createProtein(){
		...
		for (index = startPosition ; index != stopPosition; index += increment) {
			...
			if (mod == 2) {
				...
				if(isMito){
					aminoAcid = Definitions.mitoAminoAcidList[this.indexForCodonArray(codon, isForwardsStrand)];

				}else{
					aminoAcid = Definitions.aminoAcidList[this.indexForCodonArray(codon, isForwardsStrand)];
				}
				if(aminoAcid == '.'){
